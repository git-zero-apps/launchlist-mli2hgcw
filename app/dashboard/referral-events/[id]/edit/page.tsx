...
export default function EditReferralEventPage() {
  ...
  if (!record) {
    return (
      <div className="rounded-lg bg-red-50 border border-red-200 p-4">
        <p className="text-sm text-red-700">Referral Event not found.</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-2xl">
      <div className="mb-8">
        ...
        <h1 className="text-2xl font-bold text-gray-900 mt-2">Edit Referral Event</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="card space-y-6">
        ...
        <div>
          ...
          <input id="referrer_subscriber_id" name="referrer_subscriber_id" type="text" className="input" defaultValue={String(record.referrer_subscriber_id ?? "")} required />
        </div>
        <div>
          <label htmlFor="referred_subscriber_id" className="label">Referred Subscriber Id</label>
          <input id="referred_subscriber_id" name="referred_subscriber_id" type="text" className="input" defaultValue={String(record.referred_subscriber_id ?? "")} required />
        </div>

        <div className="flex items-center gap-3 pt-4 border-t">
          <button type="submit" disabled={loading} className="btn-primary">
            {loading ? "Saving..." : "Update Referral Event"}
          </button>
